...
Fonte dos dados de precipitação: https://disc.gsfc.nasa.gov/datasets/TRMM_3B43_7/summary
...
#periodos com 12 meses incompletos de dados de precipitação
(periodo_curto <- map_dbl(trmm_periodo, nrow) %>% `[`(. != 12))
...
pilha <- pilha[names(pilha) %in% names(periodo_curto) == FALSE] # remover periodos com precipitação incompleta (menos que doze meses). Retorna 'named numeric(0)' se nenhum mês é removido.

# calcular mcwd para cada periodo (usa 100 por padrão para evapotranspiraçaõ)
mcwd.periodo <- map(pilha, calc, mcwd.f2)
```
...
```{r}
hh = "anomalia" # Mudar para calculo de 1) "delta" ou 2) "anomalia"
...
map_df(mcwd.periodo, extract, SpatialPoints(tt)) %>% mutate(Plot.Code = df$Plot.Code) %>% write.csv(file = "mcwd_anual_coord.csv")

xx <- as.list(r.mcwd)
names(xx) <- gsub("X", "", names(r.mcwd))
map_df(xx, extract, SpatialPoints(tt)) %>% mutate(Plot.Code = df$Plot.Code) %>% write.csv(file = paste("mcwd_", hh, "_coord.csv", sep = ""))
```
...
levelplot(r.mcwd, at =  unique(seq(min(r.mcwd@data@min), max(r.mcwd@data@max), length = 50)), ylim=c(-20,15), xlim=c(-80.9, -41.9), 
          panel = function(...){
            panel.levelplot(...)
            sp.lines(world.shp, col = "dimgrey", lwd = 0.5)
          },
          col.regions = viridis(100), 
          names.attr=names(mcwd.periodo), main = hh)

# grafico mcwd da area de interesse
levelplot(mcwd.periodo.rasterbrick, at = unique(seq(0, min(mcwd.periodo.rasterbrick@data@min), length = 50)), ylim = c(-20,15), xlim = c(-80.9, -41.9), 
          panel = function(...){
            panel.levelplot(...)
            sp.lines(world.shp, col = "dimgrey", lwd = 0.5)
          },
          col.regions = paleta(abs(min(mcwd.periodo.rasterbrick@data@min)/10)), 
          names.attr = names(mcwd.periodo))
```
